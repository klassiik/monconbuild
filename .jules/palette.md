...
## 2024-09-04 - Decorative icons in custom components missing aria-hidden
**Learning:** While Radix UI components naturally include `aria-hidden="true"` on non-semantic icons, custom components throughout the app often do not, which causes redundant screen reader announcements.
**Action:** Ensure to add `aria-hidden="true"` whenever placing lucide-react icons in custom components where the icon's meaning is already conveyed visually or by surrounding text/labels.
