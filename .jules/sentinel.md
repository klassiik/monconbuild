...
## 2025-10-26 - [Severity classification correction for JSON-LD Injection]
**Vulnerability:** The JSON-LD XSS injection reported earlier was misclassified as HIGH severity.
**Learning:** In a static site generated context (vite-react-ssg) where the input data (faqs.js) contains hardcoded literals and no untrusted user input paths exist (no CMS or database integration for this component), an injection vulnerability in `dangerouslySetInnerHTML` is only a theoretical concern. Anyone who could exploit it already has commit access.
**Prevention:** Always verify the actual source and flow of data before classifying a vulnerability's severity. This is defense-in-depth, not a HIGH severity issue.
