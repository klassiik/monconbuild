...
## 2026-08-16 - [DoS] TypeError in `hmac.compare_digest` with non-ASCII characters
**Vulnerability:** A Denial of Service (DoS) vulnerability existed in `backend/server.py` where `hmac.compare_digest(api_key, API_SECRET_KEY)` was used without encoding the strings. If an attacker provided an `api_key` containing non-ASCII characters, it would trigger a `TypeError` and crash the request handling, potentially leading to application instability or resource exhaustion if repeatedly triggered.
**Learning:** Python's `hmac.compare_digest` throws a `TypeError: comparing strings with non-ASCII characters is not supported` when called with string arguments containing non-ASCII characters. This is a subtle Python-specific gotcha that applies to any string comparison of untrusted input using this function.
**Prevention:** Always encode strings to bytes (e.g., using `.encode('utf-8')`) before comparing them with `hmac.compare_digest`, especially when one of the strings is untrusted user input (like an API key or token).
