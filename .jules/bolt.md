...
## 2024-08-02 - [Hoisting static transformations outside components]
**Learning:** In React components like `Portfolio.jsx`, statically transforming data arrays (e.g., mapping `portfolioCategories` to `schemaProjects`) inside the functional component body causes the array to be recreated on every render. This leads to unnecessary allocations and can break memoization in downstream components (like `PortfolioPageSchema`), even when the source data (`portfolioCategories`) never changes.
**Action:** Always hoist static data transformations outside of the React component definition so they run exactly once at module load time. Provide a stable object/array reference for child components to optimize rendering.
